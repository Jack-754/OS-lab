...
int main(){
    ...
    for(int i=0; i<12288; i++){
        FFLIST.insertAtEnd(Frame(i, -1, -1));
    }
        ...
        while(L<R){
            ...
            if(!pageAvl(id, page)){
                ...
                if(FFLIST.getSize()>NFFMIN){
                    ...
                    proc[id]->pageTable[page] = head->data.number | VALID_BIT | REFERENCE_BIT;
                    FFLIST.deleteValue(head->data);
                    FFLIST.insertAtEnd(Frame(victimFrame, id, pageToReplace));
                    proc[id]->counter[page] = 0xffff;
                }
            }
        ...
        if(proc[id]->cur==m){
            ...
            for (int i = 0; i < 2048; i++) {
                if (proc[id]->pageTable[i] & VALID_BIT) {
                    ...
                    FFLIST.insertAtEnd(Frame(frame, -1, -1));  // No owner for released frames
                }
            }
        }
        else{
